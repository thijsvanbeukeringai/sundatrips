...
interface Props {
  ...
  onBook?:           (date: string, maxSpots: number, slotTime: string) => void
}
          ...
          {pickedSlot && (
            <button
              ...
              onClick={() => {
                ...
                const time = slot ? `${slot.start_time}${endTime(slot.start_time, durationHours) ? ' – ' + endTime(slot.start_time, durationHours) : ''}` : ''
                onBook?.(selected!, spots, time)
              }}
