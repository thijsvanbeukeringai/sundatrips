        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          ...
          {type !== 'stay' && type !== 'transfer' && (
            <>
              ...
              <div>
                <label className={labelClass}>{t.form.startLocation ?? 'Start / meeting point'}</label>
                <input name="start_location" type="text" defaultValue={property?.start_location ?? ''} placeholder={t.form.startLocationPH ?? 'e.g. Gili Trawangan Harbor, Main pier'} className={inputClass} />
                <p className="text-[11px] text-gray-400 mt-1">{t.form.startLocationHelper ?? 'Where guests should come for this activity'}</p>
              </div>
